...
export const adjacentCellsClearer = (cell, minefield) => {
    const adjCellMineQty = countAdjacentMines(cell)
    const adjCellFlagQty = countAdjacentFlags(cell, minefield)
    if(adjCellMineQty === adjCellFlagQty){
        cell.adjacentCells.forEach((c) => {
            if(!minefield[c.row][c.col].isFlagged){
                minefield = clearCellAndSurroudingCells(minefield[c.row][c.col], minefield)
            }
        })
    }
    return minefield
